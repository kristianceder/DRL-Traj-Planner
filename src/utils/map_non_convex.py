...
evaluated in. Such a map constitutes e.g. the initial ATR position, the goal
position and the locations of obstacles and boundaries.
"""

import random
from math import pi, radians, cos, sin
from environment_non_convex import ATR, Obstacle, Boundary, Goal, MapDescription, MapGenerator
...
def generate_map_mpc(i: Union[int, None] = None) -> MapGenerator:
    ...
    envs = [
        ...
        #     'dynamic_obstacles': [
        #         [[18.5, 18.2],[28.1, 18.2], 0.06, 0.5, 1.0, pi / 2, 12, False],
        #         [[16.775, 34.0], [22.5, 42.2], 0.07, 0.3, 0.7, pi / 2 + 0.961299, 12, False],
        #         [[44.3, 9.2], [40.5, 31.8], 0.0745, 0.6, 0.6, 0,  12, False]],
        # },   
    ]
    ...
    def generate_map() -> MapDescription:
        ...
        atr = ATR(env['start'][:2], 0, env['start'][2] + random.uniform(-0.2, 0.2), 0)
...
def generate_map_dynamic() -> MapDescription:
    ...
    atr = ATR((5, random.uniform(5, 15)), 0, random.uniform(0, 2*pi), 0)
...
def generate_map_corridor() -> MapDescription:
    ...
    atr = ATR(coords[0], 0, random.uniform(0, 2 * pi), 0)
    goal = Goal(coords[-1])
...
def generate_map_eval(i: Union[int, None] = None) -> MapGenerator:
    """
    Generates maps from the paper https://doi.org/10.1109/CASE49439.2021.9551644
    on MPC.
    ...
    envs = [
        {
            ...
            'dynamic_obstacles': [
                ...
                [[16.775, 34.0], [22.5, 42.2], 0.07, 0.3, 0.7, pi / 2 + 0.961299, 12, False]],
            'nonconvex_obstacles': [
                [[40.5, 31.8], [40.5, 31.8], 0.0745, 0.6, 0.6, 0.1]],
        },   
    ]
    ...
    def generate_map() -> MapDescription:
        ...
        obstacles = [
            *[Obstacle.create_mpc_static(coords) for coords in env.get('static_obstacles', [])],
            *[Obstacle.create_mpc_dynamic(*args) for args in env.get('dynamic_obstacles', [])],
            *[Obstacle.create_non_convex_u_shape(*args) for args in env.get('nonconvex_obstacles', [])]]
        goal = Goal(env['goal'][:2])
        atr = ATR(env['start'][:2], 0, env['start'][2] + random.uniform(-0.2, 0.2), 0)
