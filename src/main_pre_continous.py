...
from pkg_ddpg_td3.utils.map import generate_map_dynamic, generate_map_corridor, generate_map_mpc
from pkg_ddpg_td3.utils.map import generate_map_scene_1, generate_map_scene_2
...
class Metrics:
    ...
    def write_latex(self,round_digit:int=4):
        ...
        return f"&  & \\textbf{{ {self.mode} }} & {ct[0]} & {ct[1]} & {ct[2]} & {dd[0]} & {dd[1]} & {smooth[0]} & {smooth[1]} & {int(print_data['finish_time'])} & {int(print_data['success_rate']*100)} \\\\ % OK run13 var"
