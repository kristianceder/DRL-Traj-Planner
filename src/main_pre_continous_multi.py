...
import numpy as np
from shapely.geometry import Polygon, Point, LineString # type: ignore
...
class HintSwitcher:
    def __init__(self, max_switch_distance: float, min_detach_distance: float, min_detach_steps:float=5, always_on:bool=False):
...
class Metrics:
    ...
    def get_average(self, round_digit:int=4) -> dict:
        ...
        self.metric_average["success_rate"] = self.success_rate # type: ignore
...
def generate_map(scene:int=1, generator:bool=True) -> Union[list[MapDescription], list[MapGenerator]]:
    """
    MapDescription = Tuple[MobileRobot, Boundary, List[Obstacle], Goal]
    """
    if scene == 0:
        map_des_list = generate_map_scene_0()
    elif scene == 1:
        map_des_list = generate_map_scene_1()
    elif scene == 2:
    ...
    def get_map_generator(index: int) -> MapGenerator:
        def _generate_map() -> MapDescription:
            return map_des_list[index]
        return _generate_map

    if generator:
        return [get_map_generator(i) for i in range(len(map_des_list))]
    else:
        return map_des_list
